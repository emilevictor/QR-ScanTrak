<?php 
	...
	try {
		$db = new PDO("mysql:host=localhost;dbname=xitanto_urbanchallenge2011", "xitanto_uc2011", "3f8923hfsjkljaJKJ");
		$stmt = $dbh->prepare('SELECT * FROM Teams WHERE teamNum=:teamNumber');
		//Set the tNUm post parameter before execution
		$stmt->bindParam(':teamNumber', $_POST['tNum']);
		$stmt->execute();
		$stmt->setFetchMode(PDO::FETCH_INTO, new Team);
		echo $stmt->teamNum;
	}
